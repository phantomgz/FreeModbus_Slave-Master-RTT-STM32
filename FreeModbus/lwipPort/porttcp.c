...
#define MB_TCP_WRITE_TIMEOUT  1000  /* ms */
#define MB_TCP_WRITE_CYCLE  5       /* ms */
...
xMBTCPPortSendResponse( const UCHAR * pucMBTCPFrame, USHORT usTCPLength )
{
    ...
    int             res;
    int             iBytesSent = 0;
    int             iTimeOut = MB_TCP_WRITE_TIMEOUT;
    ...
    do
    {
        ...
        case -1:
            if( iTimeOut > 0 )
            {
                iTimeOut -= MB_TCP_WRITE_CYCLE;                     // MB_TCP_READ_CYCLE;
                vTaskDelay( pdMS_TO_TICKS(MB_TCP_WRITE_CYCLE) );    // usleep( MB_TCP_READ_CYCLE );
            }
            else
            {
                bAbort = TRUE;
...
prvbMBPortAcceptClient(  )
{
    ...
    if( xClientSocket != INVALID_SOCKET )
    {
        LOG("can't accept new client. all connections in use.\n" );
        bOkay = FALSE;
    }
